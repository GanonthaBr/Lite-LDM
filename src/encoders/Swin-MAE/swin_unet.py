...
class PatchEmbedding3D(nn.Module):
    def __init__(self, patch_size: int = 4, in_c: int = 1, embed_dim: int = 96, norm_layer: nn.Module = None):
        super().__init__()
        self.patch_size = patch_size
        self.proj = nn.Conv3d(in_c, embed_dim, kernel_size=(patch_size,) * 3, stride=(patch_size,) * 3)
        self.norm = norm_layer(embed_dim) if norm_layer else nn.Identity()
    ...
    def padding(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if pad_d > 0 or pad_h > 0 or pad_w > 0:
            # func.pad pads in reverse order: (W_before, W_after, H_before, H_after, D_before, D_after)
            x = func.pad(x, (0, pad_w, 0, pad_h, 0, pad_d))
        return x

    @property
    def num_patches(self):
        # Convenience property used by interpolate_pos_embed
        return None  # resolved dynamically from the input shape; override if static size is needed
...
class PatchMerging3D(nn.Module):
    def __init__(self, dim: int, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
        # 3D: 2x2x2 = 8 sub-patches are concatenated, then projected down to 2*dim
    ...
    def padding(x: torch.Tensor) -> torch.Tensor:
        ...
        pad_w = W % 2
        if pad_d > 0 or pad_h > 0 or pad_w > 0:
            # func.pad in reverse order: (C, W, H, D) pairs, before/after each
            x = func.pad(x, (0, 0, 0, pad_w, 0, pad_h, 0, pad_d))
        return x

    @staticmethod
    def merging(x: torch.Tensor) -> torch.Tensor:
        # Sample all 8 corners of each 2x2x2 neighbourhood
        x000 = x[:, 0::2, 0::2, 0::2, :]
        x001 = x[:, 0::2, 0::2, 1::2, :]
        x010 = x[:, 0::2, 1::2, 0::2, :]
        x011 = x[:, 0::2, 1::2, 1::2, :]
        ...
        x110 = x[:, 1::2, 1::2, 0::2, :]
        x111 = x[:, 1::2, 1::2, 1::2, :]
        x = torch.cat([x000, x001, x010, x011, x100, x101, x110, x111], dim=-1)
        return x
...
class PatchExpanding3D(nn.Module):
    def __init__(self, dim: int, norm_layer=nn.LayerNorm):
        ...
        self.dim = dim
        # 3D: expand to 2*dim feeds a 2x2x2 spatial split, net effect is 2x upscale per axis
        self.expand = nn.Linear(dim, 4 * dim, bias=False)
        self.norm = norm_layer(dim // 2)

    def forward(self, x: torch.Tensor):
        x = self.expand(x)
        # Split last dim into (P1, P2, P3, C): 2*dim = 2*2*2 * (dim//4)
        x = rearrange(x, 'B D H W (P1 P2 P3 C) -> B (D P1) (H P2) (W P3) C', P1=2, P2=2, P3=2)
        x = self.norm(x)
        return x


class FinalPatchExpanding3D(nn.Module):
    def __init__(self, dim: int, norm_layer=nn.LayerNorm):
        super(FinalPatchExpanding3D, self).__init__()
        self.dim = dim
        # 3D: expand to 64*dim feeds a 4x4x4 spatial split, net effect is 4x upscale per axis
        self.expand = nn.Linear(dim, 64 * dim, bias=False)
        self.norm = norm_layer(dim)

    def forward(self, x: torch.Tensor):
        x = self.expand(x)
        # Split last dim into (P1, P2, P3, C): 64*dim = 4*4*4 * dim
        x = rearrange(x, 'B D H W (P1 P2 P3 C) -> B (D P1) (H P2) (W P3) C', P1=4, P2=4, P3=4)
        x = self.norm(x)
        return x
...
class WindowAttention3D(nn.Module):
    def __init__(self, dim: int, window_size: int, num_heads: int, qkv_bias: Optional[bool] = True,
        ...
        # 3D: bias table spans (2*window_size-1)^3 relative positions
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * window_size - 1) ** 3, num_heads))
        nn.init.trunc_normal_(self.relative_position_bias_table, std=.02)

        # 3D: build relative position index over (D, H, W)
        coords_range = torch.arange(self.window_size)
        coords = torch.stack(torch.meshgrid(
            [coords_range, coords_range, coords_range], indexing="ij"))   # (3, Md, Mh, Mw)
        ...
        relative_coords[:, :, 0] += self.window_size - 1   # shift D to start from 0
        relative_coords[:, :, 1] += self.window_size - 1   # shift H
        relative_coords[:, :, 2] += self.window_size - 1   # shift W
        # encode (d, h, w) triplet into a unique scalar index
        relative_coords[:, :, 0] *= (2 * self.window_size - 1) ** 2
    ...
    def window_partition(self, x: torch.Tensor) -> torch.Tensor:
        _, D, H, W, _ = x.shape
        x = rearrange(x, 'B (Nd Md) (Nh Mh) (Nw Mw) C -> (B Nd Nh Nw) Md Mh Mw C',
                      Nd=D // self.window_size,
    ...
    def create_mask(self, x: torch.Tensor) -> torch.Tensor:
        ...
        assert D % self.window_size == 0 and H % self.window_size == 0 and W % self.window_size == 0, \
            "D, H or W is not divisible by window_size"
    ...
    def forward(self, x):
        ...
        x = self.window_partition(x)
        Bn, Md, Mh, Mw, _ = x.shape
        x = rearrange(x, 'Bn Md Mh Mw C -> Bn (Md Mh Mw) C')
        qkv = rearrange(self.qkv(x), 'Bn L (T Nh P) -> T Bn Nh L P', T=3, Nh=self.num_heads)
        q, k, v = qkv.unbind(0)
        q = q * self.scale
        attn = (q @ k.transpose(-2, -1))

        # 3D relative position bias: table is (2M-1)^3, index spans window_size^3 x window_size^3
        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(
        ...
        if mask is not None:
            ...
            attn = attn.view(Bn // nW, nW, self.num_heads,
                             Md * Mh * Mw, Md * Mh * Mw) + mask.unsqueeze(1).unsqueeze(0)
            attn = attn.view(-1, self.num_heads, Md * Mh * Mw, Md * Mh * Mw)
        attn = self.softmax(attn)
        attn = self.attn_drop(attn)
        x = attn @ v
        x = rearrange(x, 'Bn Nh (Md Mh Mw) C -> Bn Md Mh Mw (Nh C)', Md=Md, Mh=Mh)
        x = self.proj(x)
        x = self.proj_drop(x)
        x = rearrange(x, '(B Nd Nh Nw) Md Mh Mw C -> B (Nd Md) (Nh Mh) (Nw Mw) C',
                      Nd=D // Md, Nh=H // Mh, Nw=W // Mw)
...
class SwinTransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, window_size=7, shift=False, mlp_ratio=4., qkv_bias=True,
        ...
        self.attn = WindowAttention3D(dim, window_size=window_size, num_heads=num_heads, qkv_bias=qkv_bias,
...
class BasicBlock(nn.Module):
    def __init__(self, index: int, embed_dim: int = 96, window_size: int = 7, depths: tuple = (2, 2, 6, 2),
        ...
        if patch_merging:
            self.downsample = PatchMerging3D(dim=embed_dim * 2 ** index, norm_layer=norm_layer)
        else:
...
class SwinTransformerBlock3D(nn.Module):
    def __init__(self, dim, num_heads, window_size=7, shift=False, mlp_ratio=4., qkv_bias=True,
        ...
        super().__init__()
        self.norm1 = norm_layer(dim)
        # 3D: use WindowAttention3D instead of WindowAttention
        self.attn = WindowAttention3D(dim, window_size=window_size, num_heads=num_heads, qkv_bias=qkv_bias,
                                      attn_drop=attn_drop, proj_drop=drop, shift=shift)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
    ...
    def forward(self, x):
        # Forward pass is dimension-agnostic; no changes needed here
        x_copy = x
        x = self.norm1(x)
        x = self.attn(x)
        x = self.drop_path(x)
...
class BasicBlock3D(nn.Module):
    def __init__(self, index: int, embed_dim: int = 96, window_size: int = 7, depths: tuple = (2, 2, 6, 2),
        ...
        # 3D: use SwinTransformerBlock3D instead of SwinTransformerBlock
        self.blocks = nn.ModuleList([
        ...
        # 3D: use PatchMerging3D instead of PatchMerging
        if patch_merging:
            self.downsample = PatchMerging3D(dim=dim, norm_layer=norm_layer)
        else:
            self.downsample = None
...
class BasicBlockUp3D(nn.Module):
    def __init__(self, index: int, embed_dim: int = 96, window_size: int = 7, depths: tuple = (2, 2, 6, 2),
                 num_heads: tuple = (3, 6, 12, 24), mlp_ratio: float = 4., qkv_bias: bool = True,
                 drop_rate: float = 0., attn_drop_rate: float = 0., drop_path: float = 0.1,
                 patch_expanding: bool = True, norm_layer=nn.LayerNorm):
        super(BasicBlockUp3D, self).__init__()
        index = len(depths) - index - 2
        depth = depths[index]
        dim = embed_dim * 2 ** index
        num_head = num_heads[index]
        ...
        # 3D: use PatchExpanding3D instead of PatchExpanding
        if patch_expanding:
            self.upsample = PatchExpanding3D(dim=embed_dim * 2 ** index, norm_layer=norm_layer)
        else:
            self.upsample = nn.Identity()
...
class SwinUnet3D(nn.Module):
    def __init__(self, patch_size: int = 4, in_chans: int = 1, num_classes: int = 1000, embed_dim: int = 96,
                 window_size: int = 7, depths: tuple = (2, 2, 6, 2), num_heads: tuple = (3, 6, 12, 24),
                 mlp_ratio: float = 4., qkv_bias: bool = True, drop_rate: float = 0., attn_drop_rate: float = 0.,
                 drop_path_rate: float = 0.1, norm_layer=nn.LayerNorm, patch_norm: bool = True):
        super().__init__()
        ...
        # 3D: PatchEmbedding3D instead of PatchEmbedding
        self.patch_embed = PatchEmbedding3D(
            patch_size=patch_size, in_c=in_chans, embed_dim=embed_dim,
            norm_layer=norm_layer if patch_norm else None)
        self.pos_drop = nn.Dropout(p=drop_rate)
        self.layers = self.build_layers()
        # 3D: PatchExpanding3D instead of PatchExpanding
        self.first_patch_expanding = PatchExpanding3D(dim=embed_dim * 2 ** (len(depths) - 1), norm_layer=norm_layer)
        self.layers_up = self.build_layers_up()
        self.skip_connection_layers = self.skip_connection()
        self.norm_up = norm_layer(embed_dim)
        # 3D: FinalPatchExpanding3D instead of FinalPatchExpanding
        self.final_patch_expanding = FinalPatchExpanding3D(dim=embed_dim, norm_layer=norm_layer)
        # 3D: Conv3d head instead of Conv2d
        self.head = nn.Conv3d(in_channels=embed_dim, out_channels=num_classes, kernel_size=(1, 1, 1), bias=False)
        self.apply(self.init_weights)
    ...
    def build_layers(self):
        layers = nn.ModuleList()
        for i in range(self.num_layers):
            # 3D: BasicBlock3D instead of BasicBlock
            layer = BasicBlock3D(
                index=i,
                depths=self.depths,
                embed_dim=self.embed_dim,
    ...
    def build_layers_up(self):
        layers_up = nn.ModuleList()
        for i in range(self.num_layers - 1):
            # 3D: BasicBlockUp3D instead of BasicBlockUp
    ...
    def skip_connection(self):
        # Dimension-agnostic; no changes needed
        skip_connection_layers = nn.ModuleList()
        for i in range(self.num_layers - 1):
            dim = self.embed_dim * 2 ** (self.num_layers - 2 - i)
            layer = nn.Linear(dim * 2, dim)
    ...
    def forward(self, x):
        ...
        # 3D: rearrange to (B, C, D, H, W) for Conv3d head
